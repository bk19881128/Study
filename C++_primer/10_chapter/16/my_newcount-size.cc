...
biggies(vector<string> &words, vector<string>::size_type sz)
{
    ...
    vector<string>::iterator wc = 
			stable_partition(words.begin(), words.end(), [sz](const string &a) {return a.size() < sz;});

	// compute the number of elements with size >= sz 
//	vector<string>::size_type count = words.end() - wc;
    //10.20
    vector<string>::size_type count = count_if(words.begin(), words.end(),  [](const string &a) {return a.size() >= 5;});
